...
const messageAckAfterProcessed = process.env.MESSAGE_ACK_AFTER_PROCESSED;
    ...
    subscribeEventHandler({ aggregateType, onErrorHandler, onCompleteHandler }) {
      const subscription =
        ...
        eventSourcing.ensureAcknowledgeRegistry$(aggregateType, mbeKey).pipe(
          ...
          map(({ event, handlers }) => ({
            handlerObservables: handlers
            //Check if the event should be processed only on sync
            .filter(({fn, instance, processOnlyOnSync}) => !processOnlyOnSync)
            .map(({ fn, instance }) => fn.call(instance, event)),
            event
          })),
